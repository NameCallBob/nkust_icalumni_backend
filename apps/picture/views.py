...
class SelfImageViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        responses={201: openapi.Response(
            '成功創建', SelfImageSerializer), 400: '格式錯誤', 401: '未授權'}
    )
...
class CompanyImageViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        responses={200: CompanyImageSerializer(many=True), 404: "公司或成員不存在", 401: "未授權"}
    )
    @action(detail=False, methods=['get'], permission_classes=[permissions.IsAuthenticated], authentication_classes=[JWTAuthentication])
    def selfInfo(self, request):
        ...
        try:
            ...
            queryset = CompanyImage.objects.filter(company=company)
            serializer = CompanyImageSerializer(queryset, many=True)
            return Response(serializer.data)
        except Company.DoesNotExist:
            return Response({"error": "公司不存在"}, status=status.HTTP_404_NOT_FOUND)
        except AttributeError:
            return Response({"error": "成員資訊無法取得"}, status=status.HTTP_404_NOT_FOUND)

    @swagger_auto_schema(
        operation_description="創建 CompanyImage",
        request_body=CompanyImageSerializer,
        responses={201: openapi.Response(
            '成功創建', CompanyImageSerializer), 400: '格式錯誤', 404: '公司或成員不存在', 401: '未授權'}
    )
    @action(detail=False, methods=['post'], url_path='new', permission_classes=[permissions.IsAuthenticated], authentication_classes=[JWTAuthentication])
    def new(self, request):
        from apps.company.models import Company
        try:
            company = Company.objects.get(member=request.user.member)
            data = request.data.copy()
            data['company'] = company.id  # 自動抓取 company 的 ID
    ...
    def change(self, request):
        try:
            ...
            return Response({"error": "找不到指定的圖片"}, status=status.HTTP_404_NOT_FOUND)
...
class ProductImageViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        responses={201: openapi.Response(
            '成功創建', ProductImageSerializer), 400: '格式錯誤', 401: '未授權'}
    )
    @action(detail=False, methods=['post'], url_path='new', permission_classes=[permissions.IsAuthenticated], authentication_classes=[JWTAuthentication])
    def new(self, request):
        data = request.data.copy()
        data['product'] = request.data.get(
            'product_id')  # 假設前端傳遞的產品 ID 使用 `product_id`
        print(data['product'])
...
class PopupAdViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAdminOrReadOnly]  # 預設權限為任何人可讀取
